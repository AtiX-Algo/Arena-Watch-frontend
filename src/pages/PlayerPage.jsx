...
export default function PlayerPage() {
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      try {
        ...
        // Ensure we always set an array to the store, even if the API fails
        const safeData = Array.isArray(data) ? data : [];
        setChannels(safeData);
        
        if (safeData.length > 0 && !activeChannel) {
          setActiveChannel(safeData[0]); // Auto-select first channel for UX
        }
      } catch (error) {
        console.error("Error fetching channels:", error);
        setChannels([]); // Fallback to empty array on network error
      } finally {
      ...
      try {
        ...
        player.updateSettings({
          streaming: {
            ...
            retryAttempts: { manifest: 3, mediaSegment: 3 }
          }
        });
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] w-full bg-[#070708] overflow-hidden antialiased">
            ...
            channels.map(channel => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 group text-left relative border ${
                    isSelected 
                ...
                >
                  {isSelected && (
                    <span className="absolute left-0 top-3 bottom-3 w-1 bg-accent-green rounded-r" />
                  )}
                  ...
                  {isSelected && (
                    <Signal className="w-3.5 h-3.5 text-accent-green animate-pulse shrink-0" />
                  )}
                </button>
              );
            })
      ...
      {/* ================= MONITOR MATRIX (VIDEO CONTAINER) ================= */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-[#070708] h-full custom-scrollbar">
        {!activeChannel ? (
          <div className="flex flex-col items-center justify-center w-full h-full min-h-[400px] bg-[#0d0d0f]/50 rounded-2xl border border-gray-900 shadow-2xl relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(34,197,94,0.02),transparent)] pointer-events-none" />
            ...
            <p className="text-xs text-gray-600 mt-1 max-w-xs text-center">Deploy an active visual stream vector from the control module sidebar grid.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-5 w-full max-w-6xl mx-auto transition-all duration-300">
            ...
            <div className="relative w-full aspect-video bg-black rounded-2xl overflow-hidden border border-gray-900 shadow-[0_12px_40px_rgba(0,0,0,0.5)] group">
              {error && (
                <div className="absolute top-4 left-4 right-4 z-20 bg-red-950/80 backdrop-blur-md border border-red-500/30 text-white px-4 py-3 rounded-xl flex items-center gap-3 animate-fade-in shadow-xl">
            ...
            {/* Matrix Metadata & Routing Control Rack */}
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-5 bg-[#0d0d0f] rounded-2xl border border-gray-900 gap-5 shadow-xl relative overflow-hidden">
              <div className="flex items-center gap-4 min-w-0">
                <div className="w-12 h-12 rounded-xl bg-[#121215] border border-gray-800 overflow-hidden flex items-center justify-center shrink-0 relative group">
                  {activeChannel.logo ? (
                    <img src={activeChannel.logo} alt="" className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                  ) : (
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-bold mt-1.5 flex-wrap">
                    ...
                    <span className="bg-[#141417] px-2 py-0.5 rounded border border-gray-800 text-gray-400 font-mono tracking-wider">{activeServer?.quality || 'RAW-AUTO'}</span>
                  </div>
                </div>
              </div>

              {/* Redundant Signal Relays */}
                  ...
                  {activeChannel.servers.map((server, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl text-xs font-bold flex items-center transition-all duration-200 border ${
                          isActive
                            ? 'bg-accent-green border-accent-green text-black shadow-[0_0_15px_rgba(34,197,94,0.2)]'
                            : 'bg-[#121215] border-gray-800 text-gray-400 hover:bg-[#16161a] hover:text-gray-200 hover:border-gray-700'
                        }`}
                      >
